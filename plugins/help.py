...
def help(inp, say=None, notice=None, input=None, conn=None, bot=None, chan=None):
    ...
    for command, (func, args) in bot.commands.iteritems():
        ...
        if chan in bot.config["plugins"]["help"]["slimchans"]:
	    if fn.group(1).lower() not in disabled:
		if fn.group(1).lower() in bot.config["plugins"]["help"]["slimplugins"]:
    ...
    if not inp:
        ...
        if out[1]:
            notice(out[1][1:])
	if chan in bot.config["plugins"]["help"]["slimchans"]:
	    notice("For a full list of commands, message me with 'help'")
